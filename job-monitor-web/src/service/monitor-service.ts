...
import { ExecutionHistory, RunningTask } from "@/types/api";

export async function fetchAllMonitorHistory(): Promise<ExecutionHistory[]> {
  try {
    const response = await apiClient.get<BackendResponse<ExecutionHistory[]>>(
      `/monitor/history`
    );
...
export async function fetchAllRunningTasks(): Promise<RunningTask[]> {
  try {
    const response = await apiClient.get<BackendResponse<RunningTask[]>>(
      `/monitor`
    );
